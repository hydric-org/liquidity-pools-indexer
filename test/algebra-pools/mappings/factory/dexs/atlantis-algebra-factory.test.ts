...
import { AtlantisAlgebraFactory_CustomPool_event, AtlantisAlgebraFactory_Pool_event } from "generated";
import { AtlantisAlgebraFactory, MockDb } from "generated/src/TestHelpers.gen";
...
describe("AtlantisAlgebraFactory", () => {
  const mockDb = MockDb.createMockDb();
  let network: IndexerNetwork;
  let poolEvent: AtlantisAlgebraFactory_Pool_event;
  let customPoolEvent: AtlantisAlgebraFactory_CustomPool_event;

  beforeEach(() => {
    sinon.stub(algebraFactory, "handleAlgebraPoolCreated").callsFake(async (..._args: any[]) => Promise.resolve());
    network = IndexerNetwork.MONAD;

    poolEvent = AtlantisAlgebraFactory.Pool.createMockEvent({
      mockEventData: {
        chainId: network,
      },
    ...
    customPoolEvent = AtlantisAlgebraFactory.CustomPool.createMockEvent({
      mockEventData: {
        chainId: network,
      },
      pool: "0x0000000000000000000000000000000000000221",
  ...
  describe("Pool Handler", () => {
    ...
    it("should pass ATLANTIS_ALGEBRA as protocol to handler", async () => {
      ...
      assert.equal(passedProtocol, SupportedProtocol.ATLANTIS_ALGEBRA);
    });
