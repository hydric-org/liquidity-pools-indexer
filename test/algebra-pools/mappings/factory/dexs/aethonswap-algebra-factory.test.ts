...
import { AethonSwapAlgebraFactory_CustomPool_event, AethonSwapAlgebraFactory_Pool_event } from "generated";
import { AethonSwapAlgebraFactory, MockDb } from "generated/src/TestHelpers.gen";
import sinon from "sinon";
import { AlgebraVersion } from "../../../../../src/algebra-pools/common/enums/algebra-version";
import * as algebraFactory from "../../../../../src/algebra-pools/mappings/factory/algebra-factory";
...
describe("AethonSwapAlgebraFactory", () => {
  const mockDb = MockDb.createMockDb();
  let network: IndexerNetwork;
  let poolEvent: AethonSwapAlgebraFactory_Pool_event;
  let customPoolEvent: AethonSwapAlgebraFactory_CustomPool_event;

  beforeEach(() => {
    sinon.stub(algebraFactory, "handleAlgebraPoolCreated").callsFake(async (..._args: any[]) => Promise.resolve());
    network = IndexerNetwork.MONAD;

    poolEvent = AethonSwapAlgebraFactory.Pool.createMockEvent({
      mockEventData: {
        chainId: network,
      },
    ...
    customPoolEvent = AethonSwapAlgebraFactory.CustomPool.createMockEvent({
      mockEventData: {
        chainId: network,
      },
      pool: "0x0000000000000000000000000000000000000221",
  ...
  describe("Pool Handler", () => {
    ...
    it("should pass AETHONSWAP_ALGEBRA as protocol to handler", async () => {
      ...
      assert.equal(passedProtocol, SupportedProtocol.AETHONSWAP_ALGEBRA);
    });

    it("should pass V1_2_0 as version to handler", async () => {
      ...
      assert.equal(passedVersion, AlgebraVersion.V1_2_0);
    });
