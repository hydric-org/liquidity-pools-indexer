...
import { handleAlgebraPoolCollect } from "../../../../src/algebra-style-pools/mappings/pool/algebra-pool-collect";
import { PoolSetters } from "../../../../src/common/pool-setters";
import { handlerContextCustomMock, PoolMock, TokenMock } from "../../../mocks";

describe("AlgebraPoolCollectHandler", () => {
  ...
  it(`Should deduct the pool token0 tvl with the amount passed in the event`, async () => {
    ...
    await handleAlgebraPoolCollect(context, pool, token0, token1, amount0, BigInt(0), eventTimestamp, poolSetters);
  ...
  it(`should deduct the pool token1 tvl with the amount passed in the event`, async () => {
    ...
    await handleAlgebraPoolCollect(context, pool, token0, token1, BigInt(0), amount1, eventTimestamp, poolSetters);
  ...
  it(`should update the pool usd tvl after deducting the token amounts from the pool`, async () => {
    ...
    await handleAlgebraPoolCollect(context, pool, token0, token1, amount0, amount1, eventTimestamp, poolSetters);
  ...
  it("should update the pool entity with the result from 'updatePoolTimeframedAccumulatedYield'", async () => {
    ...
    await handleAlgebraPoolCollect(context, pool, token0, token1, 0n, 0n, eventTimestamp, poolSetters);
    const updatedPool = await context.Pool.getOrThrow(pool.id)!;
