...
import { PinotFinanceV3Factory_PoolCreated_event } from "generated";
import { MockDb, PinotFinanceV3Factory } from "generated/src/TestHelpers.gen";
import sinon from "sinon";
import { IndexerNetwork } from "../../../../../src/common/enums/indexer-network";
import { SupportedProtocol } from "../../../../../src/common/enums/supported-protocol";
import * as v3Factory from "../../../../../src/v3-pools/mappings/factory/v3-factory";

describe("PinotFinanceV3Factory", () => {
  const mockDb = MockDb.createMockDb();
  let network: IndexerNetwork;
  let poolEvent: PinotFinanceV3Factory_PoolCreated_event;

  beforeEach(() => {
    sinon.stub(v3Factory, "handleV3PoolCreated").callsFake(async (..._args: any[]) => Promise.resolve());
    network = IndexerNetwork.MONAD;

    poolEvent = PinotFinanceV3Factory.PoolCreated.createMockEvent({
      mockEventData: {
        chainId: network,
      },
  ...
  describe("Pool Handler", () => {
    ...
    it("should pass PINOT_FINANCE_V3 as protocol to handler", async () => {
      ...
      assert.equal(passedProtocol, SupportedProtocol.PINOT_FINANCE_V3);
    });
