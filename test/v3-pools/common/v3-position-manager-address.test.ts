...
describe("V3PositionManagerAddress", () => {
  ...
  describe("Uniswap", () => {
    ...
    it("should return the correct address for uniswap on scroll", async () => {
      assert.equal(
        V3PositionManagerAddress.uniswap(IndexerNetwork.SCROLL),
    ...
    it("should return the correct address for uniswap on plasma", async () => {
      assert.equal(
        V3PositionManagerAddress.uniswap(IndexerNetwork.PLASMA),
        "0x743E03cceB4af2efA3CC76838f6E8B50B63F184c"
      );
    });

    it("should return the correct address for uniswap on monad", async () => {
      assert.equal(
        V3PositionManagerAddress.uniswap(IndexerNetwork.MONAD),
  ...
  describe("PancakeSwap", () => {
    ...
    it("should return the correct address for pancakeSwap on monad", async () => {
      assert.equal(
        V3PositionManagerAddress.pancakeSwap(IndexerNetwork.MONAD),
        "0x46A15B0b27311cedF172AB29E4f4766fbE7F4364"
      );
    });
  ...
  describe("SushiSwap", () => {
    ...
    it("should throw when calling sushiSwap on monad", async () => {
      assert.throws(() => {
        V3PositionManagerAddress.sushiSwap(IndexerNetwork.MONAD);
      }, Error("SushiSwap is not supported on Monad"));
    });
  ...
  describe("Velodrome", () => {
    ...
    it("should throw when calling velodrome on Monad", async () => {
      assert.throws(() => {
        V3PositionManagerAddress.velodrome(IndexerNetwork.MONAD);
      }, Error(`Velodrome is not supported on Monad`));
    });
  ...
  describe("AethonSwap", () => {
    it("should return the correct address for aethonSwap on monad", async () => {
      assert.equal(
        V3PositionManagerAddress.aethonSwap(IndexerNetwork.MONAD),
        "0xB879564EE31F841d8049c21227c6109856409bc7"
      );
    });

    it("should throw when calling aethonSwap on unsupported network", async () => {
      assert.throws(() => {
        V3PositionManagerAddress.aethonSwap(IndexerNetwork.BASE);
      }, Error(`AethonSwap is not supported on ${IndexerNetwork.BASE}`));
    });
  });

  describe("Atlantis", () => {
    it("should return the correct address for atlantis on monad", async () => {
      assert.equal(
    ...
    it("should throw when calling atlantis on unsupported network", async () => {
      assert.throws(() => {
        V3PositionManagerAddress.atlantis(IndexerNetwork.BASE);
      }, Error(`Atlantis is not supported on ${IndexerNetwork.BASE}`));
    });
  });

  describe("Ramses", () => {
    it("should return the correct address for ramses on hyperevm", async () => {
      assert.equal(
        V3PositionManagerAddress.ramses(IndexerNetwork.HYPER_EVM),
        "0xB3F77C5134D643483253D22E0Ca24627aE42ED51"
      );
    });

    it("should throw when calling ramses on unsupported network", async () => {
      assert.throws(() => {
        V3PositionManagerAddress.ramses(IndexerNetwork.BASE);
      }, Error(`Ramses is not supported on ${IndexerNetwork.BASE}`));
    });
  });

  describe("OctoSwap", () => {
    it("should return the correct address for octoSwap on monad", async () => {
      assert.equal(
    ...
    it("should throw when calling octoSwap on unsupported network", async () => {
      assert.throws(() => {
        V3PositionManagerAddress.octoSwap(IndexerNetwork.BASE);
      }, Error(`OctoSwap is not supported on ${IndexerNetwork.BASE}`));
    });
  });

  describe("Pinot Finance", () => {
    it("should return the correct address for pinotFinance on monad", async () => {
      assert.equal(
    ...
    it("should throw when calling pinotFinance on unsupported network", async () => {
      assert.throws(() => {
        V3PositionManagerAddress.pinotFinance(IndexerNetwork.BASE);
      }, Error(`Pinot Finance is not supported on ${IndexerNetwork.BASE}`));
    });
  });
});
