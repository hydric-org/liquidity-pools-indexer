import { OctoSwapCLFactory } from "generated";
...
OctoSwapCLFactory.PoolCreated.contractRegister(({ event, context }) => {
  context.addUniswapV3Pool(event.params.pool);
});

OctoSwapCLFactory.PoolCreated.handler(async ({ event, context }) => {
