import { PinotFinanceV3Factory } from "generated";
...
PinotFinanceV3Factory.PoolCreated.contractRegister(({ event, context }) => {
  context.addUniswapV3Pool(event.params.pool);
});

PinotFinanceV3Factory.PoolCreated.handler(async ({ event, context }) => {
  await processV3PoolCreated({
    ...
    protocol: SupportedProtocol.PINOT_FINANCE_V3,
  });
});
