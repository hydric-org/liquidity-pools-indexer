import { CapricornCLFactory } from "generated";
...
CapricornCLFactory.PoolCreated.contractRegister(({ event, context }) => {
  context.addUniswapV3Pool(event.params.pool);
});

CapricornCLFactory.PoolCreated.handler(async ({ event, context }) => {
  await processV3PoolCreated({
    ...
    protocol: SupportedProtocol.CAPRICORN_CL,
  });
});
